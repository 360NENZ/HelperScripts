...
NAME_MAPPINGS = {
  ...
  "release_asset_index": "AssetBundles"
}
...
def parse_res(name, rel_path, base_url, dst_dir):
  ...
  while True:
    ...
    res_dir = DIR_MAPPINGS.get(os.path.splitext(remoteName)[1]) or NAME_MAPPINGS.get(remoteName) or ""
  ...
  if args.client:
    for client in CLIENTS:
      for name in CLIENT_FILES: 
        fetch_file(f"{CLIENT_BASE_PATH}/{args.variation}/output_{args.client}/client/{client}/{name}", BASE_URL, args.out)
  ...
  if args.silence:
    for client in CLIENTS:
      for name in SILENCE_FILES:
        fetch_file(f"{SILENCE_BASE_PATH}/{args.variation}/output_{args.silence}/client_silence/{client}/{name}", BASE_URL, args.out)
        parse_res(name, f"{SILENCE_BASE_PATH}/{args.variation}/output_{args.silence}/client_silence/{client}", BASE_URL, args.out)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--silence", "-s", type=str, help="Silence version")
